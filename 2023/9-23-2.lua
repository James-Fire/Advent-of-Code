...
for i, entry in ipairs(Worklist) do
	for j=#entry,1,-1 do
		...
		if AllValuesSame(WorkTable) then
			...
			table.insert(WorkTable,1,WorkTable[1])
		...
		--	table.insert(WorkTable,WorkTable[#WorkTable]+entry[j+1][#entry[j+1]])
			...
			table.insert(WorkTable,1,WorkTable[1]-entry[j+1][1])
...
for i, entry in ipairs(Worklist) do
	print("Extrapolated Value "..i.." "..tostring(entry[1][1]))
	Value = Value + entry[1][1]
end
